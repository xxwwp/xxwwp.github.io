...
| Inherited 继承:          | no                                                                                                                                                                                  |
| Percentages 百分比:      | n/a                                                                                                                                                                                 |
| Computed value 计算值:   | the keyword none or one or more of size, layout, paint （仅为 ‘none’ 或者 ‘size’，‘layout’，‘paint’ 中的复数个）                                                                    |
...
大致翻译是任何拦截 **如果被作用到 `html` 或者 body 元素上时，那么从视口到初始包含块、视口或 canvas 背景的属性将禁止传播**，这会影响到：
...
需要注意的是，paint 拦截产生的剪裁类似 `overflow:clip`，不会订内部的粘性定位元素产生影响。
